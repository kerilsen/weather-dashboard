...
$(function () {
  ...
  // Populating the seasonal photo
  // Switch statement based on the current month
  switch (dayjs().month()) {
    case 0: // January
    case 1: // February
    case 2: // March
  ...
  function renderCity(array) {
    let name = array[0];
    let pastCity = $("<li><a></a></li>")
      .text(`${name}`)
      .addClass("list-group-item pastCities");
  ...
  $("#history").click(function (event) {
    console.log(
      `history was clicked and target is ${event.target.textContent} and the class list is ${event.target.classList}`
    );
  ...
  function getCurrent(array) {
    ...
    fetch(openWeatherCurrent)
      ...
      .then(function (data) {
        ...
        console.log(
          `temp with temperature, wind and humidity pushed is ${temp}`
        );
